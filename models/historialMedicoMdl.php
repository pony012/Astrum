...
class HistorialMedicoMdl extends BaseMdl{
	...
	**/
	function updateAguaAlDia($idAguaAlDia,$idHistorialMedico,$poca,$regularAg,$mucha){
		if($stmt = $this->driver->prepare('SELECT IDAguaAlDia FROM AguaAlDia WHERE IDAguaAlDia=?')){
			...
			if($mySqliResult->field_count > 0 && $mySqliResult->fetch_assoc()['IDAguaAlDia']!=''){
				...
				if(!$stmt->bind_param('isssi',$idHistorialMedico, $this->poca,$this->regularAg,$this->mucha,$idAguaAlDia)){
					return false;
				}
				if (!$stmt->execute()) {
					return false;
	...
	* @return array or false
	**/
	function listsAguaAlDia($offset = -1,$idAguaAlDia = -1, $constrain = '1 = 1'){
		$rows = array();
		if($offset>-1){
