$(function(){
    ...
    var selectAjusteSalidaTipo = $("#idAjusteSalidaTipo");
    $.ajax({
        url: selectAjusteSalidaTipo.data("url"),
        type: "POST",
        dataType: "json",
        contentType: "application/json; charset=utf-8"
    }).done(function(response){
        $.each(response.data, function(i, ajusteSalidaTipo){
            selectAjusteSalidaTipo.append($('<option>',{
                text: ajusteSalidaTipo.Tipo,
                value: ajusteSalidaTipo.IDAjusteSalidaTipo
            }));
        });
        var selected = selectAjusteSalidaTipo.data("selected");
        if(selected){
            selectAjusteSalidaTipo.find("option:selected").removeAttr("selected");
            selectAjusteSalidaTipo.find("[value="+selected+"]").attr("selected","selected");
        }
    });
